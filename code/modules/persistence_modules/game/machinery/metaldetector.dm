...
/obj/machinery/metal_detector/New()
	...
	component_parts += new /obj/item/weapon/stock_parts/circuitboard/metal_detector(src)
...
/obj/machinery/metal_detector/attackby(obj/item/W, mob/usr)
	if(istype(W, /obj/item/weapon/card/id) || istype(W, /obj/item/modular_computer/pda))
		if(!req_access_faction && W.get_faction())
			req_access_faction = W.get_faction()
...
/obj/machinery/metal_detector/Crossed(var/atom/A)
	if(istype(A, /mob/living))
		...
		if(req_access_faction)
			if(has_access(list(core_access_security_programs), list(), M.GetAccess(req_access_faction)) && (M.get_faction() == req_access_faction))
