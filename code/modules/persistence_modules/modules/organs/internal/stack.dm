...
/mob/living/carbon/human/revive()
	...
	if(stack)
		if(stack.brainmob && stack.brainmob.mind)
			stack.brainmob.mind.transfer_to(src)
	. = ..()
...
/datum/extension/faction_state_listener/stack
	expected_type = /obj/item/organ/internal/stack

/datum/extension/faction_state_listener/stack/OnFactionClosed(var/faction_uid)
	if(!..())
		return
	return TRUE

/datum/extension/faction_state_listener/stack/OnFactionOpened(var/faction_uid)
	if(!..())
		return
	return TRUE
...
/obj/item/organ/internal/stack
	...
	var/mob/living/carbon/brain/lace/brainmob
	var/sensor = 0
...
/obj/item/organ/internal/stack/New(var/loc, var/faction_uid)
	...
	set_extension(src, /datum/extension/faction_state_listener/stack)
...
/obj/item/organ/internal/stack/Destroy(var/clearlace = FALSE)
	if(!clearlace && brainmob && ((brainmob.key && brainmob.key != "") || (brainmob.key && brainmob.key != "")))
		loc = get_turf(loc)
...
/obj/item/organ/internal/stack/proc/transfer_identity(var/mob/living/carbon/H)
	...
	brainmob.teleport_time = 	H.timeofdeath + DEAD_LACEMOB_STORAGE_TELEPORT_DELAY
	brainmob.container = 		src
	brainmob.saved_cryonet = 	H.saved_cryonet.Clone()

	if(H.mind)
		H.mind.transfer_to(brainmob)

	to_chat(brainmob, SPAN_NOTICE("You feel slightly disoriented. Your conciousness suddenly shifts into a neural lace."))
...
/obj/item/organ/internal/stack/proc/get_owner()
	if(brainmob)
		return brainmob
	if(istype(loc, /obj/item/device/mmi/lmi) && istype(loc.loc, /mob/living/silicon/robot))
		return loc.loc
	if(owner)
		return owner
	return 0
...
/obj/item/organ/internal/stack/ui_action_click()
	if(brainmob)
		ui_interact(brainmob)
	else if(istype(loc, /obj/item/device/mmi/lmi) && istype(loc.loc, /mob/living/silicon/robot))
...
/obj/item/organ/internal/stack/Topic(href, href_list)
	switch (href_list["action"])
		...
		// 	faction.clock_in(owner?.real_name)
		// 	//try_connect()
		...
		// 		var/charname = SScharacter_setup.peek_character_name(save_slot, brainmob.ckey)
		...
		// 			SScharacter_setup.delete_character(save_slot, brainmob.ckey)
		// 			var/mob/new_player/M = new /mob/new_player()
		// 			M.loc = null
		// 			M.key = brainmob.key

		// 			brainmob.perma_dead = 1
		// 			brainmob.ckey = null
		// 			brainmob.stored_ckey = null
		...
		// if("teleport")
		// 	var/obj/machinery/lace_storage/storage = GetLaceStorage(brainmob)
		// 	if (storage)
		// 		forceMove(storage)
		// 		return 1
		// 	else
		// 		message_admins("Couldnt find a Lace Storage for brainmob [brainmob] (stack.dm)")
		// 	return 0
		if("transfer")
			var/datum/money_account/source = get_money_account_by_name(owner.real_name)
			if(!source)
				to_chat(usr, "Could not retrieve your bank account!")
				return
			var/chosename = input(usr, "Enter the full name of the person you want to transfer to", "Money Transfer") as null|text
			if(chosename)
				var/datum/money_account/target = get_money_account_by_name(chosename)
				if(!target)
					to_chat(usr, "Could not retrieve [chosename]'s bank account!")
					return
				var/choseamount = input(usr, "Enter the amount you want to transfer.", "Money Transfer") as null|num
				if(choseamount > 0)
					if(choseamount > source.get_balance())
						to_chat(usr, "You do not have sufficent funds to make this transfer.")
						return
					var/src_ckey = GetCharacterRecord(source.owner_name).get_ckey()
					var/tgt_ckey = GetCharacterRecord(target.owner_name).get_ckey()
					if(src_ckey == tgt_ckey)
						to_chat(usr, "This transfer is not allowed. View the server rules.")
						message_admins("[src_ckey] attempted to transfer money fromtwo of their own charactrs ([owner.real_name] to[chosename])")
						return
					source.transfer(target, choseamount, "Money transfer from [owner.real_name] to [chosename]")
...
/obj/item/organ/internal/stack/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = GLOB.interactive_state)
	...
	//try_connect()
	if(!brainmob && owner)
		var/datum/money_account/src_account = get_money_account_by_name(user.real_name)
		data["living"] = 1
		if(menu == 1)
			var/datum/computer_file/data/email_account/eac = PSDB.email.GetEmailAccountByName(user.real_name)
			if(eac)
				data["email_status"] = "You have [eac.unread()] unread emails."
			else
				data["email_status"] = "Cannot read email account. Contact Administrator."
			if(faction)
				data["faction_name"] = faction.name
				if(duty_status == 1)
					data["work_status"] = faction.get_on_duty(user.real_name)
		...
		if(menu == 2)
			if(!src_account)
				...
				data["account_balance"] = src_account.get_balance()
				var/list/transactions = src_account.transaction_log
				...
				if(transactions.len)
					for(var/i=0; i<10; i++)
						...
						formatted_transactions[++formatted_transactions.len] = list("date" = T.date, "time" = T.time, "target_name" = T.get_target_name(), "purpose" = T.purpose, "amount" = T.amount ? T.amount : 0)
		...
		// if(menu == 3)
		// 	var/datum/world_faction/democratic/F = FindFaction(GLOB.using_map.default_faction_uid)
		// 	if(istype(F))
		...
		if(brainmob)
			if(brainmob.teleport_time < world.time)
				data["can_teleport"] = 1
			else
				data["time_teleport"] = round((brainmob.teleport_time - world.time)/(1 MINUTE))
		else
			message_admins("lace UI without owner or brainmob [src] [src.loc] ([x], [y], [z])")
...
/obj/item/organ/internal/stack/proc/get_potential()
	var/list/datum/world_faction/potential
	var/user_name
	if(owner)
		user_name = owner.real_name
	else
		var/obj/item/device/mmi/lmi/LMI = loc
		var/mob/M = LMI.get_mob()
		user_name = M ? M.real_name : null
	if(user_name)
		potential = get_associated_factions(user_name)
	return potential

//If we're in a lmi inside a robot, this returns the robot
/obj/item/organ/internal/stack/proc/get_robot()
...
//Returns the actual mob entity who owns the lace. This will return the proper mob whether in a cyborg or human
/obj/item/organ/internal/stack/proc/get_actual_mob()
	var/mob/ownermob = get_robot()
	if(!ownermob)
		ownermob = loc
	return ownermob

/obj/item/organ/internal/stack/proc/can_clock_in()
	var/mob/M = get_actual_mob()
	if(!M)
		return //no mob
	if(!faction)
		return
	var/datum/computer_file/report/crew_record/faction/R = faction.get_record(M.real_name)
	if(!R)
		return
	var/datum/assignment/A = faction.get_assignment(R.get_assignment_uid(), R.get_name())
	if(!A)
		return
	return A.can_clock_in()

// /obj/item/organ/internal/stack/proc/try_duty()
// 	var/mob/ownermob = get_actual_mob()
// 	if(!ownermob)
// 		return //no mob
// 	if(!faction)
// 		return
// 	var/datum/computer_file/report/crew_record/faction/R = faction.get_record(ownermob.real_name)
// 	if(!R)
// 		return
// 	var/datum/assignment/A = faction.get_assignment(R.get_assignment_uid(), R.get_name())
// 	if(!A)
...
// 	if(ownermob.real_name != faction.get_leadername())
// 		var/datum/computer_file/report/crew_record/faction/records //There's a member var called records too...
// 		if(!robot)
...
// 		var/datum/assignment/assignment = faction.get_assignment(records.get_assignment_uid(), records.get_name())
// 		var/title
// 		var/rank = 1
// 		if(assignment && assignment.can_clock_in())
// 			if(record)
// 				title = assignment.get_title(record.get_rank())
...
// 	if(!owner) 
// 		return FALSE
// 	faction = FindFaction(connected_faction)
// 	if(!faction || faction.status != FACTION_STATUS_FLAG_ACTIVE) 
// 		return FALSE
	
// 	var/datum/computer_file/report/crew_record/faction/record = faction.get_record(owner.real_name)
// 	if(!record)
...
// 			return 0
// 	else
// 		var/datum/assignment/assignment = faction.get_assignment(record.get_assignment_uid(), record.get_name())
...
/obj/item/organ/internal/stack/proc/do_backup()
	var/mob/living/mymob = get_actual_mob()
	if(is_broken() || mymob?.is_dead() || !(mymob?.mind))
		return
	languages = mymob.languages.Copy()
	mind_backup = mymob.mind
	default_language = mymob.default_language
	if(mymob.ckey)
		ownerckey = mymob.ckey ? mymob.ckey : mymob.stored_ckey

/obj/item/organ/internal/stack/proc/backup_inviable()
	return 	(!istype(mind_backup) || mind_backup == owner.mind || (mind_backup.current && mind_backup.current.stat != DEAD))
...
/obj/item/organ/internal/stack/proc/can_delete_stack_action()
	if( (owner == null && brainmob == null) || loc == null || QDELETED(src))
		return TRUE
	return FALSE
...
/obj/item/organ/internal/stack/proc/overwrite()
	if(owner.mind && owner.ckey) //Someone is already in this body!
		if(owner.mind == mind_backup) // Oh, it's the same mind in the backup. Someone must've spammed the 'Start Procedure' button in a panic.
			return
	...
	mind_backup.active = TRUE
	mind_backup.transfer_to(owner)
	if(default_language)
