...
/datum/species/phorosian/handle_environment_special(var/mob/living/carbon/human/H)
	...
	if(H.get_pressure_weakness() > 0.6) //If air gets in, then well there's a problem.
		...
		if(!environment)
			return
		var/oxidizermoles = environment.get_by_flag(XGM_GAS_OXIDIZER)
		if(oxidizermoles && oxidizermoles >= 0.5 && !H.on_fire) //Phorosians so long as there's enough oxygen (0.5 moles, same as it takes to burn gaseous phoron).
			...
			H.IgniteMob()
	//The base burn proc will extinguish the mob when in a low pressure environment. For other cases, extinguishing should be handled by the phorosian suit itself.
