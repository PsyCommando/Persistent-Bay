	...
	switch(href_list["action"])
		...
		if("print")
			if(last_print > world.time)
				to_chat(usr, SPAN_WARNING("You must wait before printing."))
				return
			...
			"}
			var/obj/item/weapon/paper/contract/contract = MakeMiscContract(location, t, selected_contract.name, "", usr.real_name, )
			contract.info = t
