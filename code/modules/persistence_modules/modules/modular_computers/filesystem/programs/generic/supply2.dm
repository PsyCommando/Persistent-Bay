...
#define SUPPLY_PAGE_MAIN 1
#define SUPPLY_PAGE_STATISTICS 2
#define SUPPLY_PAGE_TELEPAD 3
#define SUPPLY_PAGE_PROCESSING 4
#define SUPPLY_PAGE_EXPORTS 5
#define SUPPLY_PAGE_EXPORT_TELEPAD 6
#define SUPPLY_PAGE_REVOKE_TELEPAD 7
#define NUMBER_SUPPLY_PAGES 7
...
/datum/nano_module/program/supply/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = 1, state = GLOB.default_state)
	...
	var/is_admin = check_access(user, core_access_invoicing, connected_faction.uid)
	...
	data["screen"] = screen
	switch(screen)
		if(SUPPLY_PAGE_MAIN)// Main ordering menu
			data["categories"] = category_names
			if(selected_category)
				data["category"] = selected_category
				data["possible_purchases"] = category_contents[selected_category]

		if(SUPPLY_PAGE_STATISTICS)// Statistics screen with credit overview
		...
		if(SUPPLY_PAGE_TELEPAD) // order confirmation and telepad control
			var/list/telepads[0]
			for(var/obj/machinery/telepad_cargo/telepad in GLOB.cargotelepads)
				if(telepad.faction_uid != connected_faction.uid)
					continue
		...
		if(SUPPLY_PAGE_PROCESSING)// Order processing
			var/list/cart[0]
			var/list/requests[0]
		...
		if(SUPPLY_PAGE_EXPORTS) // export view..
			var/list/exports[0]
		...
		if(SUPPLY_PAGE_EXPORT_TELEPAD)
			...
			for(var/obj/machinery/telepad_cargo/telepad in GLOB.cargotelepads)
				...
				data["telepads"] = telepads
		if(SUPPLY_PAGE_REVOKE_TELEPAD) // revoke telepads
			var/list/telepads[0]
			for(var/obj/machinery/telepad_cargo/telepad in GLOB.cargotelepads)
				if(telepad.faction_uid != connected_faction.uid)
					continue
...
/datum/nano_module/program/supply/Topic(href, href_list)
	...
	if(href_list["order"])
		...
		connected_faction.queue_pending_order(O)
		return 1
	...
	if(href_list["revoke_pad"])
		...
		for(var/obj/machinery/telepad_cargo/telepad in selected_telepads_revoke)
			...
			telepad.disconnect_faction(user)
			revoked += 1
		...
		for(var/obj/machinery/telepad_cargo/telepad in selected_telepads_export)
			...
			for(var/obj/structure/closet/closet in T.contents)
				for(var/obj/item/weapon/paper/export/export in closet.contents)
					...
					if(earn)
						connected_faction.central_account.deposit(earn, "Export ([export.name])", "Central Authority Exports")
						earned += earn
						sent++
					break
				break
	...
	if(href_list["launch_order"])
		...
		for(var/datum/supply_order/SO in connected_faction.approved_orders)
			if(!clear_turfs.len)
				...
				connected_faction.cancel_approved_order(SO.ordernum)
				break
			var/i = rand(1,clear_turfs.len)
			var/turf/pickedloc = clear_turfs[i]
			clear_turfs.Cut(i,i+1)

			connected_faction.finish_approved_order(SO.ordernum)
	...
	if(href_list["approve_order"])
		...
		var/datum/computer_file/report/crew_record/faction/R = connected_faction.get_record(user_id_card.registered_name)
		if(!R) return 0
		var/expense_limit = connected_faction.get_member_expenses_limit(R.get_name())
		var/id = text2num(href_list["approve_order"])
		for(var/datum/supply_order/SO in connected_faction.pending_orders)
			if(SO.ordernum != id)
				continue
			if(SO.object.cost*10 > expense_limit - connected_faction.get_member_expenses(R.get_name()))
				...
				return 1
			if(SO.object.cost*10 > connected_faction.get_central_account().money)
				to_chat(usr, "<span class='warning'>Not enough [GLOB.using_map.local_currency_name] to purchase \the [SO.object.name]!</span>")
				return 1
			connected_faction.approve_pending_order(SO.ordernum, R.get_name(), TRUE)
			break
		return 1
	...
	if(href_list["invoice_order"])
		...
		for(var/datum/supply_order/SO in connected_faction.pending_orders)
			...
			invoice.raw_total = SO.object.cost*10
	...
	if(href_list["deny_order"])
		...
		connected_faction.deny_pending_order(id)
		return 1
...
/datum/nano_module/program/supply/proc/print_export(var/mob/user, var/id)
	...
	t += "<hr>"
	var/obj/item/weapon/paper/export/export = new(get_turf(program.computer.get_physical_host()))
	export.info = t
...
/datum/nano_module/program/supply/proc/print_export_business(var/mob/user, var/id)
	...
	if(order.required)
		t += "SUPPLIED/REQUESTED: [order.supplied]/[order.required]<br>"
	t += "<hr><br>"
	var/obj/item/weapon/paper/export/business/export = new(get_turf(program.computer.get_physical_host()))
	export.info = t
	export.export_id = order.id
