...
// 						if(T && istype(T))
// 							formatted_transactions[++formatted_transactions.len] = list("date" = T.date, "time" = T.time, "target_name" = T.target.owner_name, "purpose" = T.purpose, "amount" = T.amount ? T.amount : 0)
...
// 		if("print")
// 			if(!selected_law) return
// 			program.computer.print_paper(selected_law.body,selected_vote.name)
// 		if("select_criminal")
// 			law_type = 1
// 		if("select_civil")
// 			law_type = 2
