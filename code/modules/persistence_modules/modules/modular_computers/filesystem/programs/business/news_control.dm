...
/datum/nano_module/program/news_control/proc/get_file(var/filename)
	var/datum/computer_file/data/F = program?.computer?.get_file(filename)
	if(!istype(F))
		return
	return F
...
/datum/nano_module/program/news_control/proc/save_file(var/filename)
	program.computer.save_file(filename, filedata, /datum/computer_file/data/text)
	is_edited = 0
	return 1
...
/datum/nano_module/program/news_control/proc/create_file(var/newname, var/data = "")
	if(!newname)
		return
	program.computer.create_file(newname, data, /datum/computer_file/data/text)
