...
/datum/nano_module/program/faction_core/proc/try_connect()
	var/datum/world_faction/network_faction = program.get_network_faction()
	if(network_faction)
		if(connected_faction.uid == network_faction.uid && connected_faction.password == network_faction.password)
			connected = TRUE
			wrong_connection = 0
			return TRUE
	connected = FALSE
...
/datum/nano_module/program/faction_core/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = GLOB.default_state)
	...
	// 	if(menu == 13)
	// 		data["rank1_req"] = 0//connected_faction.all_promote_req
	// 		data["rank3_req"] = 0//connected_faction.three_promote_req
	// 		data["rank5_req"] = 0//connected_faction.five_promote_req
...
/datum/nano_module/program/faction_core/Topic(href, href_list)
	if(..())
		return 1
	if(!program.can_run(usr)) 
		return 1
	switch (href_list["action"])
		if("connect")
			var/datum/world_faction/fac = program?.get_network_faction()
			var/curr_faction_uid = fac.uid
			var/curr_faction_password = fac.password
			var/select_faction_uid = input(usr, "Enter Lognet UID")
			var/select_faction_password = input(usr, "Enter Lognet Password")
			if(select_faction_password == curr_faction_uid && select_faction_uid == curr_faction_password)
				if(!try_connect())
					wrong_connection = 1
			else
		...
		if("change_purpose")
			var/curr_name = connected_faction.desc
			var/select_name = sanitize(input(usr,"Enter a description or purpose for your organization.","Lognet Desc.", connected_faction.desc) as null|text, 126)
			if(select_name)
				if(curr_name != connected_faction.desc)
					to_chat(usr, "Your inputs expired because someone used the terminal first.")
				else
					connected_faction.desc = select_name
		...
		if("change_networkuid")
			var/curr_name = connected_faction.network.uid
			...
			if(select_name)
				if(curr_name != connected_faction.network.uid)
					to_chat(usr, "Your inputs expired because someone used the terminal first.")
				else
					for(var/datum/world_faction/existing_faction in GLOB.all_world_factions)
						if(existing_faction.network.uid == select_name)
							to_chat(usr, "Error! A network with that UID already exists!")
							return 1
					connected_faction.network.uid = select_name
		...
		if("hiring_command")
			connected_faction.set_hiring_allowed(FALSE)
		if("hiring_anyone")
			connected_faction.set_hiring_allowed(TRUE)
		if("menu_back")
			menu = 3
		...
		if("print_expense")
			...
			expense.owner_name = usr.get_id_name()
