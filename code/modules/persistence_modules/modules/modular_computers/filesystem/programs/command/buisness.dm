			...
			if(transactions.len)
				for(var/i=0; i<10; i++)
					...
					if(minus < transactions.len)
						...
						if(T && istype(T))
							formatted_transactions[++formatted_transactions.len] = list("date" = T.date, "time" = T.time, "target_name" = T.target, "purpose" = T.purpose, "amount" = T.amount ? T.amount : 0)
...
/datum/nano_module/program/business/Topic(href, href_list)
	...
	if(href_list["PRG_openfile"])
		...
		if(is_edited)
			if(alert("Would you like to save your changes first?",,"Yes","No") == "Yes")
				program.computer.save_file("[headline] (draft)", filedata)
		var/datum/computer_file/data/F = program.computer.get_file(href_list["PRG_openfile"])
		if(!F)
	...
	if(href_list["PRG_saveasfile"])
		...
		if(!newname)
			return 1
		var/datum/computer_file/data/F = program.computer.create_file(newname, filedata)
		if(!F)
	...
	switch(href_list["action"])
			...
			if(select_name)
				...
				for(var/datum/computer_file/report/crew_record/R in GLOB.all_crew_records)
					if(R.get_name() == select_name)
						...
						break
				if(!found)
					var/datum/computer_file/report/crew_record/L = GetGlobalCrewRecord(select_name)
					if(L) found = 1
				if(!found)
		...
		if("employee_print")
			...
			if(user_id_card.registered_name == connected_business.ceo_name)
				...
				id.selected_faction_uid = connected_business.name
				...
				id.icon = 'code/modules/persistence_modules/icons/items/extra_card.dmi'
		...
		if("expense_print")
			...
			expense.owner_name = user_id_card.registered_name
		...
		if("tax_disconnect")
			...
			if(choice == "Confirm")
				var/datum/world_faction/connected_faction = FindFaction(connected_business.tax_network)
				if(connected_faction)
		...
		if("tax_connect")
			...
			if(select_title)
				var/datum/world_faction/connected_faction = FindFaction(select_title)
				if(connected_faction)
